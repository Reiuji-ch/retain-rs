use crate::stream::{nonce_from_u128, ENCRYPTED_BLOCK_SIZE};
...
impl<S: Stream<Item = Result<bytes::Bytes, ReqwestError>>> Stream for DecryptingStream<S> {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match this.state {
            DecReadState::Nonce => {
                let res: Option<Result<bytes::Bytes, ReqwestError>> =
                ...
                match res {
                    Some(Ok(ref bytes)) => {
                        ...
                        while this.input_buffer.len() >= 3 * ENCRYPTED_BLOCK_SIZE {
                            match this.aead.decrypt(
                                &nonce_from_u128(*this.nonce),
                                &this.input_buffer[0..ENCRYPTED_BLOCK_SIZE],
                            ) {
                                Ok(mut ciphertext) => {
                                    *this.nonce += 1;
            ...
            DecReadState::Pad => {
                ...
                assert!(
                    buffered == ENCRYPTED_BLOCK_SIZE
                        || buffered == ENCRYPTED_BLOCK_SIZE * 2
                        || buffered == ENCRYPTED_BLOCK_SIZE * 3
                );
                this.output_buffer.clear();
                ...
                let pad_amount = u32::from_le_bytes(
                    match this.output_buffer[this.output_buffer.len() - 4..].try_into() {
                        Ok(n) => n,
                        Err(_err) => {
                ...
                *this.state = DecReadState::Done;
                Poll::Ready(Some(Ok(BytesMut::from(
                    &this.output_buffer[..(this.output_buffer.len() - pad_amount as usize)],
                ))))
            }
            DecReadState::Done => Poll::Ready(None),
        }
    }
}
