...
use backblaze_api::api::{b2_cancel_large_file, b2_finish_large_file, b2_start_large_file, b2_upload_part, FileInfo, PartInfo, UploadPartAuth};
...
pub async fn upload_large_file(
    ...
    let file_id = match b2_start_large_file(auth.clone(), FileInfo {
    ...
    loop {
        ...
        retry_limited!([1, 3, 5, 10, 30, 60, 600, 1800, 3600], result, {
...
pub async fn get_upload_part_auth(auth: Arc<RwLock<Option<Auth>>>, file_id: String) -> UploadPartAuth {
    let upauth;
...
pub async fn finish_large_file(auth: Arc<RwLock<Option<Auth>>>,
                           file_id: String,
                           path: PathBuf,
                           name_nonce: u128,
