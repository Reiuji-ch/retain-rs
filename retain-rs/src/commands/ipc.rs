...
impl IPCConnection {
    ...
    pub async fn send_command<T: AsRef<str>>(&mut self, command: Command, data: T) -> Result<(Response, String), String> {
        match self.raw_send_message(format!("{}|{}", command, &data.as_ref())).await {
            Ok(_) => {
            ...
            Err(err) => {
                Err(format!("Sending message failed: {}", err))
            }
        }
    }
    ...
    async fn raw_send_message<T: AsRef<str>>(&mut self, message: T) -> tokio::io::Result<()> {
        self.client.write_all(format!("{}\x04", message.as_ref()).as_bytes()).await
    }
    ...
    async fn receive_message(&mut self, timeout: Duration) -> Result<String, ReceiveError> {
        ...
        match String::from_utf8(buf) {
            Ok(s) => Ok(s),
            Err(err) => {
                Err(ReceiveError::IOError(format!("Invalid UTF-8: {err:?}")))
            }
        }
    }

    /// Try to receive a command from the client
    pub async fn receive_command(&mut self, timeout: Duration) -> Result<(Command, String), String> {
        let message = self.receive_message(timeout).await.map_err(|err| err.to_string())?;
    ...
    /// Sends a response
    pub async fn send_response<T: AsRef<str>>(&mut self, response: Response, data: T) -> tokio::io::Result<()> {
        self.raw_send_message(format!("{}|{}", response, &data.as_ref())).await
    }
    ...
    async fn client_handshake(&mut self) -> Result<(), String> {
        ...
        let line = match self.receive_message(Duration::from_secs(5)).await {
            Ok(line) => line,
            Err(err) => {
                return Err(format!("Error reading server response during handshake - {}", err))
            }
        };
        ...
        match line.as_ref() {
            ...
            msg => {
                Err(format!("Got bad response to handshake - {}", msg))
            }
        }
    }
    ...
    async fn server_handshake(&mut self) -> Result<(), String> {
        let identifier = self.receive_message(Duration::from_secs(5)).await.map_err(|e| { e.to_string() })?;
        let version = self.receive_message(Duration::from_secs(5)).await.map_err(|e| { e.to_string() })?;
        ...
        match (identifier.as_ref(), version.as_ref()) {
            ...
            (x, y) => {
                Err(format!("Handshake failed - Expected: {} {} - Got: {} {}", HANDSHAKE_IDENTIFIER, env!("CARGO_PKG_VERSION"), x, y))
            }
        }
    }
