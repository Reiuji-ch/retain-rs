...
pub async fn b2_authorize_account(key: &str) -> Result<crate::Auth, ApiError> {
    ...
    let response = client.get("https://api.backblazeb2.com/b2api/v2/b2_authorize_account")
    ...
    if response.allowed.name_prefix.is_some() {
        return Err(ApiError::BadBucketConfig("API key is restricted to a prefix, remove it."));
    }

    if response.allowed.bucket_id.is_none() {
        return Err(ApiError::BadBucketConfig("API key is not restricted to a single bucket. It should be, for safety reasons."));
    }

    if response.allowed.bucket_name.is_none() {
        return Err(ApiError::BadBucketConfig("Bucket is invalid. Has it been deleted?"));
    }
    ...
    Ok(crate::Auth {
        ...
        bucket_id: response.allowed.bucket_id.expect("This should never happen"),
        bucket_name: response.allowed.bucket_name.expect("This should never happen"),
    })
}
